...
export function initAiScan() {
  ...
  const sceneEl = document.querySelector(".ai-scan__scene");
  const scanWrap = document.querySelector(".scan-object-wrap");
  ...
  // Tarama bitince telefonu sahnenin ortasına kaydırmak için gereken yatay
  // öteleme: flatlay + aradaki boşluğun yarısı. Dar ekranda (column düzen)
  // yatay kaydırma anlamsız → 0.
  const centerShift = () => {
    if (!window.matchMedia("(min-width: 861px)").matches) return 0;
    const styles = getComputedStyle(sceneEl);
    const gap = parseFloat(styles.columnGap || styles.gap) || 0;
    return (scanWrap.offsetWidth + gap) / 2;
  };
  ...
  if (prefersReduced) {
    ...
    gsap.set(phoneWrap, { x: centerShift() });
  ...
  // 3) Lazer birinci geçiş: yukarıdan aşağı tarar (elma görünür kalır)
  tl.to(laser, { opacity: 1, duration: 0.04 }, 0.30)
    .fromTo(laser, { top: "0%" }, { top: "100%", ease: "power1.inOut", duration: 0.18 }, 0.30);

  // 4) Aşağı geçiş biter — ilk ekran (Review Detected Items) belirir.
  //    Elma/flatlay hâlâ sahnede, ikinci geçişi bekliyor.
  tl.to(screenAdd, { opacity: 0, ease: "power1.in", duration: 0.12 }, 0.36)
    .to(screenDetails, {
      opacity: 1,
      filter: "blur(0px)",
      scale: 1,
  ...
  // 5) Lazer ikinci geçiş: aşağıdan yukarı geri tarar
  tl.to(laser, { top: "0%", ease: "power1.inOut", duration: 0.18 }, 0.60)
    .to(laser, { opacity: 0, duration: 0.05 }, 0.78);

  // 5b) Flatlay tamamen kaybolduktan SONRA (0.78) telefon sahnenin ortasına
  //     süzülür — böylece telefon hâlâ görünür fotoğrafla çakışmaz. Sincap
  //     birlikte gelir (ikisi de .phone-wrap içinde).
  tl.to(phoneWrap, {
    x: () => centerShift(),
    ease: "power2.inOut",
    duration: 0.18,
  ...
  // 5c) Flatlay'in altındaki mor halka (scan-ring) da telefon ortaya süzülürken
  //     silinir — geride yüzen mor iz kalmasın.
  tl.to(scanRing, { opacity: 0, ease: "power1.in", duration: 0.12 }, 0.78);

  // 6) Flatlay, lazer yolun yarısına geldiğinde solmaya başlar (0.69 =
  //    yukarı geçişin ortası) ve tepeye ulaştığı an (0.78) tamamen kaybolur.
  //    Ardından Pantry Inventory ekranı belirir.
  tl.to(apple, {
    opacity: 0,
    scale: 0.9,
    filter: "blur(10px)",
  ...
  // 7) Sincap görevini tamamladı — küçük bir "başardım" sıçraması
  tl.to(squirrel, { y: -14, ease: "power1.out", duration: 0.08 }, 0.88)
    .to(squirrel, { y: 0, ease: "bounce.out", duration: 0.1 }, 0.94);
}
